...
return new class extends Migration
{
    ...
    {
        Schema::create('sprf_current_projects', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreign('prepared_by_user_id', 'sprf_cur_prep_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('director_customer_engagement_user_id', 'sprf_cur_dce_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('esd_director_user_id', 'sprf_cur_esd_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('vp_ccto_user_id', 'sprf_cur_vp_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('president_ceo_user_id', 'sprf_cur_pres_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('current_approver_user_id', 'sprf_cur_curr_fk')
                ->references('id')->on('users')->nullOnDelete();
    ...
    {
        Schema::dropIfExists('sprf_current_projects');
    }
};