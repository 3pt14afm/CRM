...
return new class extends Migration
{
    ...
    {
        Schema::create('sprf_entry_projects', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreign('prepared_by_user_id', 'sprf_ent_prep_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('director_customer_engagement_user_id', 'sprf_ent_dce_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('esd_director_user_id', 'sprf_ent_esd_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('vp_ccto_user_id', 'sprf_ent_vp_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('president_ceo_user_id', 'sprf_ent_pres_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('current_approver_user_id', 'sprf_ent_curr_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('approved_by_user_id', 'sprf_ent_appr_fk')
                ->references('id')->on('users')->nullOnDelete();

            $table->foreign('rejected_by_user_id', 'sprf_ent_rej_fk')
                ->references('id')->on('users')->nullOnDelete();
    ...
    {
        Schema::dropIfExists('sprf_entry_projects');
    }
};