...
import SprfItemsTable2 from '@/Components/sprf/SprfItemsTable2';
...
// SPRF2 difference: Selling Price/unit is the input, Mark-up % is derived.
// See calculations2.js for the formula and rationale.
import { computeGroup2 as computeGroup } from '@/utils/sprf/calculations2';
import { flattenItemsFromApi2 } from '@/utils/sprf/normalizers2';
...
const [companyInfo, setCompanyInfo] = useState({
  ...
  useEffect(() => {
    ...
    setCompanyInfo({
      ...
      deadline: sourceProject?.company_info?.deadline ?? null,
    ...
    setItems(flattenItemsFromApi2(sourceProject?.items ?? []));
  ...
  const handleSaveDraft = () => {
    ...
    router.post(ziggyRoute('sprf.entry2.draft.save'), formData, {
      forceFormData: true, 
      preserveScroll: true,
      preserveState: true,
      onSuccess: () => {
  ...
  const handleSubmit = () => {
    ...
    switch (true) {
      ...
      case hasMarkupOrSellingError:
        setHasAttemptedItemsSubmit(true);
        toast.error('Please enter a valid selling price. Enter 0 if no selling price.', { duration: 5000 });
        return;
    ...
    router.patch(
      ziggyRoute('sprf.entry2.projects.submit', sourceProject.id),
      buildPayload(),
      {
        preserveScroll: true,
        preserveState: true,
...
Entry2.layout = (page) => <AuthenticatedLayout children={page} />;
