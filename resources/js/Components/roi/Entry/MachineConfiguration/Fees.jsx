...
const Fees = () => {
  const initialData = [
    { id: 1, label: '', cost: 0, qty: 0, total: 0, remarks: '', isStatic: false },
  ];

  const [rows, setRows] = useState(initialData);

  // // LOG ALL USER INPUTTED DATA
  // // This will trigger every time the 'rows' state changes
  // useEffect(() => {
  //   console.log("Current Table Data:", rows);
  // }, [rows]);
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                {/* Actions Column - Tightened Padding */}
                <td className="border-b border-r border-slate-100 p-1">
                  <div className="flex gap-1 justify-center items-center h-8">
                    <button 
                      onClick={addRow} 
                      className="w-7 h-7 flex items-center justify-center rounded bg-green-50 text-green-600 border border-green-200 hover:bg-green-100 transition-colors"
                    >
                      +
                    </button>
                    <button 
                      ...
                      className={`w-7 h-7 flex items-center justify-center rounded border transition-colors ${
                        rows.length <= 1 ? "border-slate-100 bg-slate-50 text-slate-300" : "border-red-200 bg-red-50 text-red-600 hover:bg-red-100"
                      }`}
                    >
                      -
                    </button>
